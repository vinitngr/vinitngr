...
const Sidebar = ({ isOpen, setIsOpen, selected , setselectfxn } : { isOpen: boolean, setIsOpen: (option: boolean) => void, selected: string , setselectfxn: (option: string) => void}) => {
  ...
  const onClick = () => {
    window.open("https://app.enhancv.com/share/a16b2ada/?utm_medium=growth&utm_campaign=share-resume&utm_source=dynamic", "_blank");
  };
  ...
  return (
    <div className={`bg-[#121218] font-mono lg:relative h-screen fixed top-0 z-20 left-0 border-r-2 overflow-hidden transition-all ease-in-out duration-500 border-[#2d2d3a] ${isOpen ? 'w-full lg:w-[80vw]' : 'w-0'}`}>
      ...
      <div className={`bg-[#121218] text-white max-h-screen p-5 transition-all ease-in-out duration-150 overflow-y-scroll`}>
