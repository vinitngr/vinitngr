...
function FeaturedCard({ animatedItems, handleMouseEnter, handleMouseLeave }: AboutPrevProps) {
  ...
  return (
    <div
      id="card1"
      className={`grid-item col-span-3 sm:col-span-2 min-h-64 row-span-2 bg-[#1a1a22] border border-[#2d2d3a] rounded-md p-2 transition-all duration-300 relative overflow-hidden shadow-md cursor-pointer flex flex-col justify-between
    ...
    >
      <div className="flex transition-transform duration-500 ease-in-out h-[100%]" style={{ transform: `translateX(-${currentIndex * 100}%)` }}>
        {Featured.map(({ id, title, description, tags , link }) => (
          <div key={id} className="w-full flex-shrink-0 px-5 bg-[#1a1a22] rounded-md transition-all duration-300 relative overflow-hidden cursor-pointer flex flex-col justify-between">
            <h3 className="text-lg font-semibold mb-2 tracking-tight">{title}</h3>
            <p className="text-sm text-[#a0a0a0] leading-relaxed mb-3 line-clamp-3">{description}</p>
            <div className="flex flex-wrap mb-2">
              {tags.map(tag => <span key={tag} className="inline-block bg-[#2a2a36] text-[#a0a0a0] text-xs py-1 px-2 rounded mr-1 mb-1">{tag}</span>)}
            </div>
            <a
            href={link}
      ...
      <div className="absolute bottom-2 left-1/2 transform -translate-x-1/2 flex space-x-2">
        {Featured.map((_, index) => <button key={index} onClick={() => setCurrentIndex(index)} className={`size-1 rounded-full transition-all duration-300 hover:scale-[3] hover:bg-yellow-600 ${currentIndex === index ? "bg-yellow-600 scale-[2]" : "bg-[#2a2a36]"}`} />)}
      </div>
    </div>
  );
}
